...
def mandarTerminados(procActual,indiceMP):
    ...
    listaMP[indiceMP]["Fragmentacion Interna"] = 0
...
def actualizar_estado_Proceso(proceso: Dict) -> Optional[str]:
    ...
    if any(p.get("id") == pid for p in listaTerminados):
        return "TERMINADO"
    if any(((p.get("id") == pid)and(p.get("t_arribo") <= T_Simulacion))for p in listaNuevos):
        return "PENDIENTE"
...
def mostrarColaListos():  #ezequiel
    ...
    cols = [
        ...
        ("T. Ingreso", None),
        ("T. Irrupcion", "green"),
        ("T. Restante de CPU", "green"),
        ("T. Total en cola de Listos", None),
    ]
...
def mostrarCPU():  #ezequiel
    ...
    for h, style in [("ID Proceso", "yellow"), ("Tamaño", None), ("Particion", None), ("T. Restante de CPU", "green"), ("T. Irrupcion", "green"),]:
        ...
        for proceso in listaListos:
            ...
            if proceso.get("CPU"):
                ...
                table.add_row(
                    ...
                    str(proceso.get("t_irrupcion", "xxx")),
                )
                break
    else:
        table.add_row(*["xxx"] * 5)
...
def mostrarInforme(): #agustin
    ...
    print(f"Tiempo de simulación final: >>>>>>>>>>>>>>>> {T_Simulacion} (ut) <<<<<<<<<<<<<<<<")
    mostrarTerminados()
    #Saltar renglón
    print("\n")  
...
while len(listaTerminados) < len(listaNuevos):
    ...
    procesoEjecucion = listaMP[indice_procesoEjecucion]["Proceso_alojado"] #elijo el proceso a ejecutar en CPU
    ...
    while (procesoEjecucion is not None) and (procesoEjecucion.get("t_RestanteCPU", 0) > 0):
        ...
        # Verificar si llegó un nuevo proceso para admisión en este ciclo y completar memoria principal
        ADMICION_MULTI_5()
        ...
        # control de APROPIACION de CPU por la admision de nuevos procesos causado por ADMICION_MULTI_5
